...
const StyleSelector = ({ value, onChange, isOpen, setIsOpen }: { value: ThumbnailStyle; onChange: (style: ThumbnailStyle) => void; isOpen: boolean; setIsOpen: (open: boolean) => void }) => {
  ...
  return (
    <div className="relative space-y-3 dark">
        ...
        <button type="button" onClick={()=> setIsOpen(!isOpen)} className="flex items-center justify-between w-full px-4 py-3 border text-left transition border-white/10 rounded-md bg-white/8 hover:bg-white/12">
            ...
            <ChevronDownIcon className={["h-4 w-4 text-zinc-400 transition-transform", isOpen && "rotate-180"].join(" ")}/>
        </button>
         ...
         <div className="absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg">
            {thumbnailStyles.map((style) => (
                <button
                    ...
                    className="flex items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30 w-full">
