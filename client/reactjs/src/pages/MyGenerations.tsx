  ...
  return (
   ...
   <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
     ...
     <div className="mb-8">
        ...
        <p className="text-zinc-400 text-sm mt-1">Manage and View all your AI-generated thumbnails</p>
     </div>
       ...
       {loading && (
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="animate-pulse border-white/10 border bg-white/6 h-[260px]"></div>
          ))}
         </div>
       )}
            ...
            {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                <div key={thumb._id} onClick={() => navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                  ...
                  {/* IMAGE */}
                  <div className={`relative rounded-t-2xl overflow-hidden ${aspectClass} bg-black`}>
                    {thumb.image_url ? (
                      <img src={thumb.image_url} alt={thumb.title} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" />
                    ) : (
                    ...
                    {thumb.isGenerating && <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-white text-sm font-medium">Generating...</div>}
                  </div>
                  {/* CONTENT */}
                  <div className="p-4 space-y-2">
                    <h3 className="text-lg font-semibold text-zinc-200">{thumb.title}</h3>
                  ...
                  <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                    ...
                    <DownloadIcon onClick={()=>handleDownload(thumb.image_url!, thumb.title)} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all"/>
